...
对于一个任意的简单多边形，可以用扫描线算法进行填充；对于一个凸多边形，类似扫描线，但只需要保存每一个y值对应的x值对就可以填充，不需要扫描线那么麻烦，性能上要由于扫描线。  
...
namespace KayAlgorithm
{
    public class PolygonScanning
    {
        ...
        public static List<Vector2> DrawPolygonScaleFlat(List<Vector2> convexSrc, float scale)
        {
            List<List<Vector2>> temp = DrawPolygonScale(convexSrc, scale);
            List<Vector2> result = new List<Vector2>();
            GeoUtils.FlatList(temp, ref result);
            return result;
        }
    ...
    public class ConvexPolygonScanning
    {
        ...
        //private static byte OOCE_EMPTY = 0;
        //private static byte OOCE_PARTIAL = 1;
