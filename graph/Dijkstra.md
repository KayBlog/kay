...
可以先看一个算法演示[视频](http://v.youku.com/v_show/id_XMjQyOTY1NDQw.html)   

Dijkstra算法：  
1. 数据存储在一个二级表：第一级为顶点，第二级为与之相连的顶点以及权重值   
2. 计算最短路径时，基于一个cost矩阵：如果有n个顶点，则矩阵为n x n。初始化时，所有的数据为无限大。若起点是i，则第i列的数据设置为0  
3. 每行遍历：当前点从起点开始，将当前点连接的点的权重值与当前权重值相加，如果小于cost矩阵对应的权重值，则需要做处理：将权重值更新，且将这对点信息保存起来(后面再选择下一个点作为当前节点时，根据权重值最小的来)  
4. 选出权重值最小的作为下一个当前点进行第二轮相同的计算方式   
5. 直到遍历行道末尾行  
6. 求到end的最短距离，则先获得end节点所在的列，从最后一行开始，得到一个权重值；然后从后往前遍历行，知道权重值不想等，则确定下一个顶点，然后重复上面的步奏，直到第0行。  

具体的算法实现参见下面代码(支持有向图无向图):   
```
using System;
